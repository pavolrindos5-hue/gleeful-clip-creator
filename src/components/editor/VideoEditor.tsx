...
type LeftTool =
  "select" | "cut" | "crop" | "layers" | "speed" | "presets" | "transitions" | "effects";
...
const AI_TOOLS = [
  {
    ...
    label: "Auto Enhance",
    icon: Sparkles,
    desc: "AI analyzuje každý snímok a automaticky vylepší jas, kontrast a ostrosť",
    detail: "Využíva neurónové siete trénované na 10 miliónoch videí",
  },
  {
    id: "denoise",
    label: "AI Denoising",
    icon: Wind,
    desc: "Odstraňuje digitálny šum a zrno pomocou hlbokého učenia",
    detail: "Rozlišuje skutočný obsah od šumu s presnosťou 98.7%",
  },
  {
    id: "stabilize",
    label: "AI Stabilizácia",
    icon: Zap,
    desc: "Opravuje trasenie kamery v reálnom čase bez čiernych okrajov",
    detail: "Predpovedá pohyb 5 snímkov dopredu pre plynulý výsledok",
  },
  {
    id: "colorgrade",
    label: "Color Grade",
    icon: Palette,
    desc: "Profesionálna farebná korekcia štýlom filmových štúdií",
    detail: "Trénované na filmoch z Hollywoodu, Bollywoodu a Kórey",
  },
  {
    id: "captions",
    label: "AI Titulky",
    icon: Type,
    desc: "Automatické rozpoznávanie reči a generovanie titulkov",
    detail: "Podporuje 50+ jazykov vrátane slovenčiny s 95% presnosťou",
  },
] as const;
...
const PRESETS: { id: string; label: string; filter: string; swatch: string }[] = [
  {
    id: "cinematic",
    label: "Cinematic",
    filter: "contrast(1.15) saturate(1.1) brightness(0.96) sepia(0.08)",
    swatch: "from-slate-700 to-amber-700",
  },
  {
    id: "vivid",
    label: "Vivid",
    filter: "saturate(1.6) contrast(1.1)",
    swatch: "from-fuchsia-500 to-amber-400",
  },
  {
    id: "cold",
    label: "Chladné",
    filter: "saturate(1.05) hue-rotate(-15deg) brightness(1.02)",
    swatch: "from-cyan-500 to-blue-700",
  },
  {
    id: "warm",
    label: "Teplé",
    filter: "sepia(0.25) saturate(1.2) brightness(1.04)",
  ...
  },
  {
    ...
    swatch: "from-neutral-200 to-neutral-700",
  },
  {
    id: "vintage",
    label: "Vintage",
    filter: "sepia(0.45) contrast(0.95) saturate(0.85)",
    swatch: "from-yellow-700 to-stone-600",
  },
];
...
const INTRO_TEMPLATES = [
  {
    ...
    overlay: { subtitle: "Predstavuje", bg: "from-black via-primary/30 to-black" },
  },
  {
    label: "INTRO",
    color: "from-cyan-600 to-blue-500",
    overlay: { subtitle: "Predstavuje", bg: "from-black via-cyan-500/30 to-black" },
  },
  {
    label: "INTRO",
    color: "from-amber-500 to-orange-500",
    overlay: { subtitle: "Predstavuje", bg: "from-black via-amber-500/30 to-black" },
  },
  {
    label: "INTRO",
    color: "from-emerald-500 to-green-600",
    overlay: { subtitle: "Predstavuje", bg: "from-black via-emerald-500/30 to-black" },
  },
  {
    label: "INTRO",
    color: "from-rose-500 to-pink-600",
    overlay: { subtitle: "Predstavuje", bg: "from-black via-rose-500/30 to-black" },
  },
];

// ── Živá ukážka prechodu (ako v PowerDirectore) ──
const TRANSITION_ANIM: Record<
  string,
...
> = {
  ...
  flash: {
    initial: { opacity: 0, filter: "brightness(6)" },
  ...
  glitch: {
    initial: { opacity: 0, x: "12%", skewX: 18 },
...
export default function VideoEditor({ videoUrl, videoName, isImage = false }: VideoEditorProps) {
  ...
  const [introOverlay, setIntroOverlay] = useState<{
    title: string;
    subtitle: string;
    bg: string;
  } | null>(null);
  ...
  const [playingTransition, setPlayingTransition] = useState<{ id: string; key: number } | null>(
    null,
  );
  ...
  const { activeClip, activeClipStart, activeClipIndex } = useMemo(() => {
    ...
    for (let i = 0; i < timelineClips.length; i++) {
      ...
      if (currentTime >= cum && currentTime < cum + c.duration)
        return { activeClip: c, activeClipStart: cum, activeClipIndex: i };
      cum += c.duration;
    }
    const last = timelineClips[timelineClips.length - 1];
    return {
      activeClip: last ?? null,
      activeClipStart: Math.max(0, cum - (last?.duration ?? 0)),
      activeClipIndex: Math.max(0, timelineClips.length - 1),
    };
  }, [timelineClips, currentTime]);
  const prevClipIndexRef = useRef(activeClipIndex);

  const previewClip =
    activeClip && activeClip.src ? activeClip : (timelineClips.find((c) => c.src) ?? null);

  const addMediaFile = useCallback((file: File) => {
    const isImg = file.type.startsWith("image/");
    const isVid =
      file.type.startsWith("video/") ||
      /\.(mp4|mov|webm|avi|mkv|m4v|wmv|flv|3gp)$/i.test(file.name);
    if (!isImg && !isVid) {
      alert("Nepodporovaný formát. Použi video alebo obrázok.");
      return;
    }
  ...
  useEffect(() => {
    ...
    if (currentTime < s || currentTime > s + len) {
      a.pause();
      return;
    }
  ...
  const handleTimeUpdate = () => {
    if (videoRef.current && !isScrubbing)
      setCurrentTime(activeClipStart + videoRef.current.currentTime);
  };
  ...
  const handlePlayheadPointerDown = useCallback(
    (e: React.PointerEvent) => {
      ...
      const onMove = (ev: PointerEvent) => {
        ...
        edgeAutoScroll(ev.clientX, updateFromX);
      };
      const onUp = () => {
  ...
  const handleCut = () => {
    if (!timelineClips.length) {
      toast.error("Na časovej osi nie sú žiadne klipy.");
      return;
    }
    let cumDur = 0,
      targetIdx = -1,
    ...
    for (let i = 0; i < timelineClips.length; i++) {
      const c = timelineClips[i];
      if (currentTime >= cumDur && currentTime < cumDur + c.duration) {
        targetIdx = i;
        splitOffset = currentTime - cumDur;
        break;
      }
      cumDur += c.duration;
    }
    ...
    const MIN = 0.2;
    if (src.duration < MIN * 2) {
      toast.error("Klip je príliš krátky na rozdelenie.");
      return;
    }
    ...
    const partB: TimelineClip = {
      ...
      id: Date.now(),
      label: src.label + " B",
      duration: src.duration - splitOffset,
    };
    setTimelineClips((prev) => {
      const n = [...prev];
      n.splice(targetIdx, 1, partA, partB);
      return n;
    });
  ...
  const handleClipPointerDown = (clipId: number, e: React.PointerEvent) => {
    ...
    let didDrag = false;
    const onMove = (ev: PointerEvent) => {
      if (!didDrag && Math.abs(ev.clientX - startX) > 6) didDrag = true;
      if (didDrag) {
        const upd = (x: number) => setDragState({ clipId, startX, currentX: x });
        upd(ev.clientX);
    ...
    const onUp = (ev: PointerEvent) => {
      ...
      if (didDrag) {
        ...
        if (track) {
          ...
          let cum = 0,
            insertIdx = timelineClips.length;
          for (let i = 0; i < timelineClips.length; i++) {
  ...
  const handleTrimEnd = (clipId: number, e: React.PointerEvent) => {
    ...
    const onMove = (ev: PointerEvent) => {
      const upd = (x: number) => {
        ...
        setTimelineClips((prev) =>
          prev.map((c) =>
            c.id === clipId ? { ...c, duration: Math.max(1, startDur + delta) } : c,
          ),
        );
      };
      upd(ev.clientX);
  ...
  const handleMusicPointerDown = (id: number, e: React.PointerEvent) => {
    ...
    const onMove = (ev: PointerEvent) => {
      const upd = (x: number) => {
        ...
        setMusicClips((prev) =>
          prev.map((c) => (c.id === id ? { ...c, start: Math.max(0, baseStart + delta) } : c)),
        );
      };
      upd(ev.clientX);
      edgeAutoScroll(ev.clientX, upd);
      ...
      const upd = (x: number) => {
        ...
        setMusicClips((prev) =>
          prev.map((c) => {
            if (c.id !== id) return c;
            if (edge === "end")
              return { ...c, start: baseStart, duration: Math.max(1, baseDur + delta) };
  ...
  const handleExport = useCallback(async () => {
    ...
    if (!clips.length) {
      alert("Najprv nahraj video alebo obrázok.");
      return;
    }
    setExporting(true);
    setExportPct(0);
  ...
  const mediaTransform =
    `${appliedTools.has("stabilize") ? "scale(1.06) " : ""}${cropZoom !== 1 ? `scale(${cropZoom})` : ""}`.trim() ||
    undefined;
  ...
  return (
    <motion.div
      ...
      className="flex flex-col h-[100dvh] bg-background text-foreground font-sans overflow-hidden"
    >
      {/* Top Bar */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-primary/10 bg-card/60 backdrop-blur-md shrink-0">
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1 text-muted-foreground hover:text-foreground transition-colors p-1 rounded-lg hover:bg-primary/10"
          >
            <ChevronLeft className="w-4 h-4" />
            <span className="text-xs hidden sm:inline">Späť</span>
          </button>
        ...
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex gap-1">
            {[...appliedTools].map((t) => {
              const tool = AI_TOOLS.find((x) => x.id === t);
              return tool ? (
                <span
                  key={t}
                  className="text-[10px] font-bold bg-primary/15 text-primary px-2 py-0.5 rounded-full border border-primary/20 flex items-center gap-1"
                >
      ...
      <div className="flex flex-1 overflow-hidden min-h-0 relative">
          ...
          {LEFT_TOOLS.map(({ id, icon: Icon, label, desc }) => (
            <div key={id} className="relative group/tip">
              <button
                ...
                onMouseEnter={() => setTooltipTool(id)}
                ...
                {tooltipTool === id && (
                  ...
                  >
                    <div className="bg-card border border-primary/20 rounded-lg px-2.5 py-1.5 shadow-xl whitespace-nowrap">
                      ...
                      <p className="text-[10px] text-muted-foreground max-w-[160px] whitespace-normal">
                        {desc}
                      </p>
                    </div>
                  </motion.div>
          ...
          <AnimatePresence>
            {leftTool === "speed" && (
              <motion.div
                ...
                exit={{ opacity: 0 }}
                className="absolute bottom-12 left-1 right-1"
              >
                <div className="bg-card border border-primary/30 rounded-lg p-1 text-center">
                  ...
                  {[0.5, 1, 1.5, 2].map((r) => (
                    <button
                      ...
                      className={`w-full text-[9px] py-0.5 rounded mb-0.5 font-bold transition-all ${playbackRate === r ? "bg-primary text-white" : "text-muted-foreground hover:text-primary"}`}
                    >
                      {r}×
                    </button>
                  ))}
        ...
        <AnimatePresence>
          {leftTool === "transitions" && (
            ...
            >
              ...
              {selectedGap === null ? (
                <p className="text-[9px] text-primary/70 bg-primary/5 border border-primary/15 rounded-lg px-2 py-1.5 mb-2">
                  Klikni na <span className="font-bold">∿</span> medzi klipmi v časovej osi
                </p>
              ) : (
                <p className="text-[9px] text-green-400/80 bg-green-400/5 border border-green-400/15 rounded-lg px-2 py-1.5 mb-2">
                  Prechod pred klipom <span className="font-bold">{selectedGap + 1}</span>
                </p>
              )}
                ...
                {TRANSITIONS.map((t) => {
                  ...
                  return (
                    <button
                      key={t.id}
                      onClick={() => {
                        playTransitionPreview(t.id);
                        if (selectedGap !== null) {
                          setClipTransitions((prev) =>
                            prev[selectedGap] === t.id
                              ? Object.fromEntries(
                                  Object.entries(prev).filter(([k]) => Number(k) !== selectedGap),
                                )
                              : { ...prev, [selectedGap]: t.id },
                          );
                        }
                      }}
                      className={`flex flex-col items-center gap-1 p-1 rounded-lg border text-center transition-all ${isActive ? "border-primary bg-primary/20 text-primary shadow-[0_0_8px_-2px_rgba(124,58,237,0.5)]" : "border-primary/10 bg-card/60 hover:border-primary/40 hover:bg-primary/5 text-muted-foreground"}`}
                      title={t.label}
                    >
          ...
          {leftTool === "effects" && (
            ...
            >
                ...
                return (
                  <div key={key} className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-mono text-primary">{sliders[key]}</span>
                    </div>
                    <input
                      type="range"
                      min={0}
              ...
              <div className="pt-1 border-t border-primary/10 space-y-1.5">
                ...
                {AI_TOOLS.filter((t) => ["enhance", "denoise", "colorgrade"].includes(t.id)).map(
                  ({ id, label, icon: Icon }) => {
                    const isApplied = appliedTools.has(id);
                    return (
                      <button
                        key={id}
                        onClick={() => runAITool(id)}
                        className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg border text-xs font-semibold transition-all ${isApplied ? "border-green-500/40 bg-green-500/10 text-green-400" : "border-primary/15 bg-card/60 hover:border-primary/40 text-muted-foreground hover:text-primary"}`}
                      >
                        {isApplied ? <Check className="w-3 h-3" /> : <Icon className="w-3 h-3" />}
                        {label}
                      </button>
                    );
                  },
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {CROP_RATIOS.map((r) => (
                  <button
                    key={r.id}
                    onClick={() => setCropRatio(cropRatio === r.id ? null : r.id)}
                ...
                <button
                  ...
                  className="text-[10px] font-bold py-1.5 rounded-lg border border-primary/15 bg-card/60 text-muted-foreground hover:text-primary hover:border-primary/40"
                >
                  Reset
                </button>
              </div>
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-xs font-medium">
                  <span>Priblíženie</span>
                  <span className="text-[10px] font-mono text-primary">{cropZoom.toFixed(2)}×</span>
                </div>
                <input
                  type="range"
                  min={100}
              ...
              <p className="text-[9px] text-muted-foreground">
                Orez sa prejaví v náhľade aj v exportovanom videu.
              </p>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          {leftTool === "layers" && (
            ...
            >
              ...
              {timelineClips.length === 0 && (
                <p className="text-[10px] text-muted-foreground">Žiadne klipy.</p>
              )}
              {timelineClips.map((c, i) => {
                ...
                return (
                  <div
                    key={c.id}
                    className={`rounded-lg border p-2 space-y-1.5 ${selectedClipId === c.id ? "border-primary/50 bg-primary/5" : "border-primary/10 bg-card/60"}`}
                  >
                    <div className="flex items-center gap-1.5">
                      <button
                        onClick={() => setSelectedClipId(c.id)}
                        className="flex-1 text-left text-[11px] font-semibold truncate text-foreground"
                      >
                        {i + 1}. {c.label}
                      </button>
                      <button
                        ...
                        className={`text-[10px] px-1.5 py-0.5 rounded ${hidden ? "bg-red-500/15 text-red-400" : "bg-primary/10 text-primary"}`}
                      >
                        {hidden ? "skryté" : "vid."}
                      </button>
                      <button
                        ...
                        className="text-muted-foreground hover:text-primary disabled:opacity-30 text-[11px] px-1"
                      >
                        ▲
                      </button>
                      <button
                        title="Dole"
                        disabled={i === timelineClips.length - 1}
                        onClick={() =>
                          setTimelineClips((prev) => {
                            const n = [...prev];
                            const [m] = n.splice(i, 1);
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        value={op}
                        onChange={(e) =>
                          setClipOpacity((p) => ({ ...p, [c.id]: Number(e.target.value) }))
                        }
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {PRESETS.map((p) => (
                  ...
                  >
                    <div
                      className={`w-full h-8 rounded bg-gradient-to-br ${p.swatch} mb-1`}
                      style={{ filter: p.filter }}
                    />
                    <span
                      className={`text-[9px] font-semibold ${activePreset === p.id ? "text-primary" : "text-muted-foreground"}`}
                    >
                      {p.label}
                    </span>
                  </button>
              ...
              <button
                onClick={() => {
                  ...
                  setSliders({ jas: 50, kontrast: 50, sytost: 50 });
                }}
                className="w-full text-[10px] font-semibold py-1.5 rounded-lg border border-primary/15 text-muted-foreground hover:text-primary"
              >
                Zrušiť predvoľbu
              </button>
            </motion.div>
        ...
        <div
          className="flex-1 flex flex-col bg-black relative overflow-hidden min-w-0"
          style={{ cursor: leftTool === "crop" ? "nwse-resize" : "default" }}
        >
          <div className="flex-1 relative overflow-hidden min-h-0">
            {previewClip?.src ? (
              <motion.div
                ...
                initial={
                  playingTransition
                    ? (TRANSITION_ANIM[playingTransition.id] ?? TRANSITION_ANIM.fade).initial
                    : false
                }
                animate={
                  playingTransition
                    ? (TRANSITION_ANIM[playingTransition.id] ?? TRANSITION_ANIM.fade).animate
                    : { opacity: 1 }
                }
                ...
                >
                  {previewClip.type === "image" ? (
                    <img
                      key={previewClip.id}
                      src={previewClip.src}
                      className={`w-full h-full ${cropAspect ? "object-cover" : "object-contain"}`}
                      style={{
                        filter: videoFilter,
                        transform: mediaTransform,
                        opacity: previewOpacity,
                      }}
                      alt={previewClip.label}
                    />
                  ) : (
                    <video
                      key={previewClip.id}
              ...
              <div className="w-full h-full flex items-center justify-center">
                <div className="text-center space-y-3 opacity-40">
                  <Film className="w-20 h-20 mx-auto text-primary/40" />
                  <p className="text-muted-foreground text-sm">Žiadne video nevybrané</p>
                </div>
              </div>
            )}
                ...
                >
                  ...
                  >
                    ...
                    <p className="text-white/60 text-xs uppercase tracking-[0.3em] mb-2">
                      {introOverlay.subtitle}
                    </p>
                    <h2 className="text-white text-3xl sm:text-5xl font-bold tracking-tight drop-shadow-2xl">
                      {introOverlay.title}
                    </h2>
                ...
                >
                  ...
                  >
                    ...
                    <span className="absolute -top-5 left-0 text-[10px] font-bold text-white/80">
                      {cropRatio
                        ? CROP_RATIOS.find((r) => r.id === cropRatio)?.label
                        : "Pôvodný pomer"}{" "}
                      · {cropZoom.toFixed(2)}×
                    </span>
                  </div>
            ...
            <AnimatePresence>
              {activeTool && (
                <motion.div
                  ...
                  className="absolute top-3 right-3 flex items-center gap-1.5 bg-violet-900/80 border border-violet-400/40 rounded-full px-3 py-1 text-xs text-violet-200 font-semibold"
                >
              ...
              <div className="absolute top-2 left-2 flex gap-1 flex-wrap">
                {[...appliedTools].map((t) => {
                  ...
                  return tool && Icon ? (
                    <span
                      key={t}
                      className="text-[10px] font-bold bg-black/60 border border-primary/40 text-primary px-2 py-0.5 rounded-full flex items-center gap-1 backdrop-blur-sm"
                    >
          ...
          <div className="shrink-0 px-3 py-2 bg-black/60 backdrop-blur-md border-t border-white/5">
            <div
              className="w-full h-1.5 bg-white/10 rounded-full cursor-pointer relative group mb-2"
              onClick={(e) => {
                const rect = e.currentTarget.getBoundingClientRect();
                const pct = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width));
                seekTo(pct * duration);
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={togglePlay}
                  className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center text-white shadow-[0_0_15px_-3px_rgba(124,58,237,0.8)] hover:scale-110 transition-all"
                >
                  {isPlaying ? (
              ...
              <div className="flex items-center gap-1">
                ...
                <input
                  ...
                  className="w-16 accent-violet-500 cursor-pointer hidden sm:block"
                />
              </div>
              {playbackRate !== 1 && (
                <span className="text-[10px] font-bold text-fuchsia-400 bg-fuchsia-400/10 border border-fuchsia-400/30 px-1.5 py-0.5 rounded-full">
                  {playbackRate}×
                </span>
              )}
              ...
              <div className="flex items-stretch border-b border-primary/10 shrink-0">
                {panelTabs.map(({ id, label, icon: Icon }) => (
                  <button
                    ...
                    className={`flex items-center gap-1 px-2.5 py-2.5 text-[11px] font-semibold whitespace-nowrap transition-all border-b-2 flex-1 justify-center ${activePanel === id ? "text-primary border-primary" : "text-muted-foreground border-transparent hover:text-foreground"}`}
                  >
                ...
                <button
                  ...
                  title="Skryť panel"
                  className="px-2.5 flex items-center justify-center text-muted-foreground hover:text-primary hover:bg-primary/10 border-b-2 border-transparent transition-all shrink-0"
                >
                  ...
                  {activePanel === "ai" && (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="text-xs font-bold text-foreground">AI Nástroje</p>
                      </div>
                      <p className="text-[10px] text-muted-foreground -mt-2 mb-3 leading-relaxed">
                        Klikni na nástroj — aplikuje sa na náhľad aj do exportu. Ďalším klikom ho
                        vypneš.
                      </p>
                      {AI_TOOLS.map(({ id, label, icon: Icon, desc, detail }) => {
                        const isApplied = appliedTools.has(id);
                        const isRunning = activeTool === id;
                        const pct = toolProgress[id] ?? 0;
                        return (
                          <button
                            key={id}
                            onClick={() => runAITool(id)}
                            disabled={isRunning || (!!activeTool && !isApplied)}
                          ...
                          >
                            <div className="flex items-center gap-2 mb-0.5">
                              <div
                                className={`w-6 h-6 rounded-lg flex items-center justify-center shrink-0 ${isApplied ? "bg-green-500/20" : "bg-primary/10"}`}
                              >
                                {isApplied ? (
                              ...
                              {isApplied && (
                                <span className="ml-auto text-[9px] font-bold text-green-400 bg-green-400/10 px-1.5 py-0.5 rounded-full">
                                  HOTOVO
                                </span>
                              )}
                            </div>
                        ...
                        return (
                          <div key={key} className="space-y-2">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-xs font-mono text-primary">{sliders[key]}</span>
                            </div>
                            <input
                              type="range"
                              min={0}
                  ...
                  {activePanel === "clips" && (
                    ...
                    >
                      ...
                      {timelineClips.map((clip) => (
                        <div
                          key={clip.id}
                          className="flex items-center gap-2 p-2.5 rounded-xl border border-primary/10 bg-card hover:border-primary/30 transition-all group"
                        >
                          <div
                            className={`w-7 h-7 rounded-lg bg-gradient-to-br ${clip.color} flex items-center justify-center shrink-0`}
                          >
                      ...
                      <button
                        ...
                        className="w-full p-2 rounded-xl border border-dashed border-primary/20 text-muted-foreground hover:border-primary/50 hover:text-primary transition-all text-xs flex items-center justify-center gap-2"
                      >
                        <Plus className="w-3.5 h-3.5" /> Pridať klip
                      </button>
                    </motion.div>
                  )}
                  {activePanel === "audio" && (
      ...
      <div className="border-t border-primary/10 bg-background shrink-0 select-none">
        <div className="flex items-center justify-between px-3 py-1 border-b border-primary/5">
          ...
          <div className="flex items-center gap-0.5">
            <button
              ...
              className="p-1 rounded hover:bg-primary/10 text-muted-foreground hover:text-foreground transition-all"
            >
              <ZoomOut className="w-3 h-3" />
            </button>
            <span className="text-[9px] text-muted-foreground w-8 text-center">
            ...
            <button
              ...
              className="p-1 rounded hover:bg-primary/10 text-muted-foreground hover:text-foreground transition-all ml-0.5"
            >
        ...
        <div className="flex items-stretch">
          <div className="w-8 shrink-0 flex flex-col border-r border-primary/5">
            <div className="h-4" />
            <div className="h-16 flex flex-col items-center justify-center gap-0.5">
              <Film className="w-3 h-3 text-muted-foreground/40" />
              <span className="text-[7px] text-muted-foreground/40 uppercase">V1</span>
            </div>
            {appliedTools.has("captions") && (
              <div className="h-7 flex flex-col items-center justify-center gap-0.5">
                <Type className="w-2.5 h-2.5 text-cyan-400/50" />
              </div>
            )}
            <div className="h-10" />
          </div>
          ...
          <div ref={timelineScrollRef} className="flex-1 overflow-x-auto overflow-y-hidden">
            {(() => {
              const totalClipW = timelineClips.reduce(
                (s, c) => s + Math.round(c.duration * PX_PER_SEC * timelineZoom),
                0,
              );
              ...
              return (
                <div
                  style={{ width: Math.max(contentW, 100), minWidth: "100%" }}
                  className="relative"
                >
                  ...
                  <div className="h-4 relative border-b border-primary/5 bg-background">
                    {Array.from({ length: numTicks }).map((_, i) => (
                      <div
                        ...
                        style={{ left: i * rulerStep * PX_PER_SEC * timelineZoom - 1 }}
                      >
                        <div className="w-px h-2 bg-primary/20" />
                        <span className="text-[7px] text-muted-foreground/40 ml-1">
                          {i * rulerStep}s
                        </span>
                      </div>
                    ))}
                  ...
                  <div
                    ...
                    onClick={(e) => {
                      if (
                        ...
                        (e.target as HTMLElement).classList.contains("timeline-clickzone")
                      )
                        handleTimelineTrackClick(e);
                      ...
                      if (idx > 0) {
                        ...
                        parts.push(
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              setLeftTool("transitions");
                              setSelectedGap(idx);
                            }}
                            title={tDef ? tDef.label : "Pridať prechod"}
                      ...
                      parts.push(
                        <div
                          ...
                          onPointerDown={(e) => {
                            if (!(e.target as HTMLElement).dataset.handle)
                              handleClipPointerDown(clip.id, e);
                          }}
                        ...
                        >
                          {clip.thumbUrl ? (
                            ...
                            <div
                              className={`w-full h-full bg-gradient-to-r ${clip.color} flex items-center justify-center`}
                            >
                              <Film className="w-5 h-5 text-white/20 pointer-events-none" />
                            </div>
                          )}
                          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent px-1.5 pb-1 pointer-events-none">
                            <span className="text-[8px] text-white font-semibold truncate block">
                              {clip.label}
                            </span>
                          </div>
                          <div className="absolute top-1 right-1 bg-black/50 rounded px-1 pointer-events-none">
                            <span className="text-[7px] text-white/60">
                              {Math.round(clip.duration)}s
                            </span>
                          </div>
                          {isSel && (
                            <div className="absolute inset-0 border-2 border-amber-400 pointer-events-none z-10 rounded-sm" />
                          )}
                          {isSel && (
                            <button
                              onPointerDown={(e) => {
                          ...
                          <div
                            ...
                            onPointerDown={(e) => {
                              e.stopPropagation();
                              handleTrimStart(clip.id, e);
                            }}
                            className={`absolute left-0 top-0 bottom-0 w-6 cursor-ew-resize z-20 flex items-center justify-center group/trim transition-all ${isSel ? "bg-amber-400/90" : "bg-amber-400/0 hover:bg-amber-400/40"}`}
                          >
                            <div
                              className={`w-0.5 h-8 rounded-full pointer-events-none transition-all ${isSel ? "bg-black/60" : "bg-amber-400/0 group-hover/trim:bg-amber-400"}`}
                            />
                          </div>
                          <div
                            ...
                            onPointerDown={(e) => {
                              e.stopPropagation();
                              handleTrimEnd(clip.id, e);
                            }}
                            className={`absolute right-0 top-0 bottom-0 w-6 cursor-ew-resize z-20 flex items-center justify-center group/trim transition-all ${isSel ? "bg-amber-400/90" : "bg-amber-400/0 hover:bg-amber-400/40"}`}
                          >
                            <div
                              className={`w-0.5 h-8 rounded-full pointer-events-none transition-all ${isSel ? "bg-black/60" : "bg-amber-400/0 group-hover/trim:bg-amber-400"}`}
                            />
                  ...
                  <AnimatePresence>
                    {appliedTools.has("captions") && (
                      <motion.div
                        ...
                        className="h-7 bg-cyan-500/5 border-b border-primary/5 flex items-center px-2 gap-1.5"
                        style={{ minWidth: contentW }}
                      >
                        <Type className="w-2.5 h-2.5 text-cyan-400 shrink-0" />
                        <div className="flex-1 h-3 bg-cyan-500/20 rounded-full overflow-hidden">
                          <div className="h-full w-3/4 bg-cyan-400/40 rounded-full" />
                        </div>
                        <span className="text-[8px] text-cyan-400/60 shrink-0">
                          {SUBTITLE_LINES.length} seg.
                        </span>
                      </motion.div>
                    )}
                  </AnimatePresence>
                    ...
                    >
                      {musicClips.length === 0 ? (
                        <button
                          ...
                          className="flex-1 border-b border-dashed border-primary/15 text-muted-foreground/40 hover:text-primary hover:border-primary/40 transition-all text-xs flex items-center justify-center gap-1.5"
                        >
                          <Plus className="w-3 h-3" />
                          <span className="text-[9px]">Pridať hudbu</span>
                        </button>
                      ) : (
                        musicClips.map((mc) => {
                          ...
                          const widthPx = Math.max(
                            24,
                            Math.round((mc.duration ?? totalDuration) * PX_PER_SEC * timelineZoom),
                          );
                          return (
                            <div
                              key={mc.id}
                              onPointerDown={(e) => {
                                if (!(e.target as HTMLElement).dataset.handle)
                                  handleMusicPointerDown(mc.id, e);
                              }}
                              onClick={(e) => e.stopPropagation()}
                              style={{
                                position: "absolute",
                            ...
                            >
                              <div
                                className="flex gap-px w-full items-end pointer-events-none"
                                style={{ height: 20 }}
                              >
                                {WAVEFORM.slice(0, 50).map((h, i) => (
                                  <div
                                    key={i}
                              ...
                              <span className="absolute bottom-0.5 left-2 text-[7px] text-white/50 font-medium truncate max-w-[80%] pointer-events-none">
                                {mc.label}
                              </span>
                              {mSel && (
                                <div className="absolute inset-0 border-2 border-amber-400 pointer-events-none z-10" />
                              )}
                              {mSel && (
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setMusicClips((prev) => prev.filter((c) => c.id !== mc.id));
                                    setSelectedMusicId(null);
                                  }}
                                  className="absolute -top-2 -right-2 z-40 w-6 h-6 flex items-center justify-center rounded-full bg-red-500 hover:bg-red-600 text-white shadow-[0_2px_8px_rgba(0,0,0,0.4)] hover:scale-110 transition-all"
                              ...
                              <div
                                data-handle="m-trim-start"
                                style={{ touchAction: "none" }}
                                onPointerDown={(e) => {
                                  e.stopPropagation();
                                  handleMusicTrim(mc.id, e, "start");
                                }}
                              ...
                              >
                                <div
                                  className={`w-0.5 h-5 rounded-full pointer-events-none ${mSel ? "bg-black/60" : "bg-amber-400/0 group-hover/mtrim:bg-amber-400"}`}
                                />
                              </div>
                              <div
                                data-handle="m-trim-end"
                                style={{ touchAction: "none" }}
                                onPointerDown={(e) => {
                                  e.stopPropagation();
                                  handleMusicTrim(mc.id, e, "end");
                                }}
        ...
        <AnimatePresence>
          {(selectedClipId !== null || selectedMusicId !== null) && (
            <motion.div
              ...
              exit={{ opacity: 0, height: 0 }}
              className="flex items-center gap-1 px-3 py-1.5 border-t border-primary/10 overflow-hidden"
            >
              {selectedClipId !== null && (
                <>
                  <button
                    onClick={handleCut}
                    className="flex flex-col items-center gap-0.5 px-3 py-1 rounded-lg hover:bg-white/5 text-muted-foreground hover:text-foreground transition-all"
                  >
                    <Scissors className="w-3.5 h-3.5" />
                    <span className="text-[9px]">Strih</span>
                  </button>
                  <button
                    onClick={() => setLeftTool("speed")}
                    className="flex flex-col items-center gap-0.5 px-3 py-1 rounded-lg hover:bg-white/5 text-muted-foreground hover:text-foreground transition-all"
                  >
                    <Clock className="w-3.5 h-3.5" />
                    <span className="text-[9px]">Rýchlosť</span>
                  </button>
                </>
              )}
              ...
              <button
                ...
                className="flex flex-col items-center gap-0.5 px-3 py-1 rounded-lg hover:bg-red-500/10 text-muted-foreground hover:text-red-400 transition-all"
              >
                <Trash2 className="w-3.5 h-3.5" />
                <span className="text-[9px]">Odstrániť</span>
              </button>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      />
      {musicTrack?.src && (
        <audio ref={musicElRef} src={musicTrack.src} className="hidden" preload="auto" loop />
      )}
      <AnimatePresence>
        {showMusicModal && (
          <>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold">Pridať hudbu</span>
                </div>
                <button
                  onClick={() => setShowMusicModal(false)}
                  className="p-1 text-muted-foreground hover:text-foreground hover:bg-primary/10 rounded-lg transition-all"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              <p className="text-[11px] text-muted-foreground mb-4">
                Vyber zdroj hudby pre tvoje video
              </p>
              <button
                onClick={() => musicAudioRef.current?.click()}
                className="w-full flex items-center gap-3 p-3.5 rounded-xl border border-primary/15 bg-card hover:border-primary/50 hover:bg-primary/5 transition-all group"
              >
                <div className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center shrink-0 group-hover:bg-primary/20 transition-all">
                  <FileMusic className="w-4 h-4 text-primary" />
                </div>
                <div className="text-left">
                  <p className="text-xs font-semibold">Zo súboru</p>
                  <p className="text-[10px] text-muted-foreground">MP3, WAV, AAC, FLAC…</p>
                </div>
